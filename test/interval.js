...
runTest('start + duration derives end', test => {
...
runTest('end + duration derives start', test => {
	const duration = new Duration('PT2H')
...
runTest('changing start rebuilds duration when end is set', test => {
	const duration = new Duration()
		.setStart(new Date('2024-01-01T00:00:00Z'))
...
runTest('rebuild() recomputes duration from start and end', test => {
...
runTest('start is undefined without end or duration', test => {
	const duration = new Duration()
	expect(duration.start, 'to be undefined')
	expect(duration.end, 'to be undefined')
})

runTest('start + end produces correct ISO string', test => {
	const duration = new Duration()
		.setStart(new Date('2024-01-01T00:00:00Z'))
